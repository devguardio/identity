...
func (self *Secret) SignPrehashed(subject string, sha512 []byte) (*Signature, error) {
	...
	sig, err := ed25519.NewKeyFromSeed(self[:]).Sign(rand.Reader, sha512, &ed25519.Options{
		Hash:    crypto.SHA512,
		Context: subject,
	})
	if err != nil {
...
func (self *Secret) SignContext(subject string, message []byte) (*Signature, error) {
	...
	sig, err := ed25519.NewKeyFromSeed(self[:]).Sign(rand.Reader, message, &ed25519.Options{
		Context: subject,
	})
	if err != nil {
		return nil, err
